...
1. For the binding process to succeed, the host supervisor domain manager must
   ...
   calling the `sbi_covh_add_tvm_device_region()` `COVH` ABI. The TSM services
...
7. Through regular discovery mechanisms (ACPI, PCI bus scanning), the TVM
   ...
   The TDI MMIO ranges are mapped into the TVM address space by the the host
   security domain manager, through the `sbi_covh_add_tvm_device_region()`
   `COVH` ABI.
...
6. **TDI IO ranges**: The TVM will likely interact with and program the TDI
   ...
   GPA ranges, through `sbi_covh_add_tvm_device_region()` `COVH` calls. The TVM
   then verifies from the TSM that a GPA exposed TDI MMIO range will be mapped
   to the TDISP reported range through the TSM managed G-stage page tables, by
...
note over TVM,TSM: Check Device Interface MMIO mappings
    ...
    loop For all TDISP reported MMIO ranges
         ...
         TSM ->> TSM: Compare with the VMM donated MMIO regions (sbi_covh_add_tvm_device_region)
