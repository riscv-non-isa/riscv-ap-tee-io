...
The verifier itself can either run locally or be hosted on an online service
...
| Endorsement (Root CA Cert, CRL) | Provisioned in TVM (Static or Dynamic) | Online Service
| Reference Value (RIM)           | Provisioned in TVM (Static or Dynamic) | Online Service
| Policy Update (Endorsement, Reference) | No update or scheduled update (Pull or Push) | Always up-to-date
| Verifier Location               | Inside of TVM         | Inside of TVM or Online Service
|===
...
the proxy connecting to the verifier service. It should also include the online
...
| Device Policy Data in TVM report   | Endorsement and Reference Value    | Online Service URL and Public Cert (trust anchor)
