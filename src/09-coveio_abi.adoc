...
The IDE key descriptor at `key_desc_addr` must be structured as follows:
...
structure as the `KEY_PROG` data object payload, as described in the `IDE_KM`
section of the PCIe specification:

[#table_sbi_covt_rp_ide_key_data]
.CoVE IDE Key and IV
[options=header]
|===
| *Byte Offset* | *Field* | *Size (bytes)* | *Description*
| 00h           | Key_DW0 | 4              | IDE Key DWord 0
| 04h           | Key_DW1 | 4              | IDE Key DWord 1
| 08h           | Key_DW2 | 4              | IDE Key DWord 2
| 0Ch           | Key_DW3 | 4              | IDE Key DWord 3
| 10h           | Key_DW4 | 4              | IDE Key DWord 4
| 14h           | Key_DW5 | 4              | IDE Key DWord 5
| 18h           | Key_DW6 | 4              | IDE Key DWord 6
| 1Ch           | Key_DW7 | 4              | IDE Key DWord 7
| 20h           | IV_DW0  | 4              | IV DWord 0
| 24h           | IV_DW1  | 4              | IV DWord 1
|===
...
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
| SBI_ERR_INVALID_ADDRESS | `key_desc_addr` or `key_data_addr` is invalid.
| SBI_ERR_INVALID_PARAM   | The key descriptor or `rp_id` is invalid.
...
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
| SBI_ERR_INVALID_ADDRESS | `key_desc_addr` is invalid.
| SBI_ERR_INVALID_PARAM   | `rp_id` is invalid.
...
[#table_sbi_covt_disable_rp_ide_key]
...
enum sbi_covt_ide_key_status {
     ...
     /* The key is programmed but is not enabled */
     Programmed,
