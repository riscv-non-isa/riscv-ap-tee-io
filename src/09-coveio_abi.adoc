...
Programs a PCIe IDE key into the `rp_id` PCIe Root Port.
...
The IDE key and IV data at `key_data_addr` must be structured as follows:
...
Calling `sbi_covt_program_rp_ide_key()` with a key descriptor that has already
been used will overwrite the existing key content.

Overwriting a key that has been enabled through `sbi_covt_enable_rp_ide_key()`
returns an `SBI_INVALID_PARAM` error.
...
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
| SBI_ERR_INVALID_PARAM   | The key is already programmed and enabled.
| SBI_ERR_DENIED          | The key can not be programmed or stored.
| SBI_ERR_FAILED          | The operation failed for unknown reasons.
|===
...
Clears a previously programmed IDE key for the `rp_id` PCIe Root Port.

Clearing a key that that has been enabled through `sbi_covt_enable_rp_ide_key()`
returns an `SBI_INVALID_PARAM` error.
...
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
| SBI_ERR_INVALID_PARAM   | The key is enabled.
| SBI_ERR_FAILED          | The operation failed for unknown reasons.
|===


[#sbi_covt_enable_rp_ide_key]
===== Function: CoVE TSM Enable PCIe Root Port IDE Key (FID #1026)
[source, C]
-----
struct sbiret sbi_covt_enable_rp_ide_key(unsigned long rp_id,
                                         unsigned long key_desc_addr);
-----

Enables a previously programmed IDE key for the `rp_id` PCIe Root Port.
...
[#table_sbi_covt_enable_rp_ide_key]
.CoVE TSM Enable PCIe Root Port IDE Key
[cols="2,3", width=90%, align="center", options="header"]
|===
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
| SBI_ERR_INVALID_PARAM   | The `key_desc_addr` address points to an unknown key.
| SBI_ERR_FAILED          | The operation failed for unknown reasons.
|===

[#sbi_covt_disable_rp_ide_key]
===== Function: CoVE TSM Disable PCIe Root Port IDE Tx Key (FID #1027)
[source, C]
-----
struct sbiret sbi_covt_disable_rp_ide_key(unsigned long rp_id,
                                          unsigned long key_desc_addr);
-----

Disables a previously enabled IDE key for the `rp_id` PCIe Root Port.

Once disabled, a key can be cleared.
