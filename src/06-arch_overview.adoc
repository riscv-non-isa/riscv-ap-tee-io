...
and is the confidential Supervisor Domain manager. It uses the <<Smmpt>>
specified Memory Protection Table (MPT) to enforce TVM memory isolation from
...
domains managing TVMs to which TDIs can be bound to. CoVE-IO uses the <<Smmpt>>
and <<IO-MPT>> extensions, together with the IOMMU instance memory-mapped
register programming interface (`RPI` in figure 1) to grant TDIs with direct
