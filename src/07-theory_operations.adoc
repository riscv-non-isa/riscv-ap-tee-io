...
Register Programming Interface (RPI), which could range from an actual IOMMU
instance to an MTT-enforced, memory-mapped access to a physically partitioned
IOMMU.
...
* May include optional TDISP messages exchanged by the TSM (e.g.
  `TDISP_GET_VERSION` or `TDISP_GET_CAPABILITY`).
...
[TDISP_GET_CAPABILITY][TDISP_CAPABILITY]
```

Once the connection is established, the TSM returns the complete connection
transcript to the host security domain manager via the
...
Management protocol.
Here again, the host supervisor domain manager implicitly initiates the PCI IDE
...
4. Refreshing the PCIe root port IDE keys for a given stream ID, upon the host
   security domain manager request.
...
9. The TSM, for each Rx sub-stream (3 of them), triggers IDE in the physical
   device’s PCIe root port by calling the `sbi_covt_enable_rp_ide_key()`
   `COVT` function.
...
Loop 3 times (For each sub-stream)
     TSM ->> RDSM: [COVT] - sbi_sbi_covt_enable_rp_ide_key(rp_id, key_desc)
...
6. The TSM, for each Tx sub-stream, disables the corresponding IDE key
   ...
   the `sbi_covt_disable_rp_ide_key()` `COVT` function.
...
Loop 3 times - TSM requests the RP to stop using the IDE stream Tx key (For each sub-stream)
     TSM ->> RDSM: [COVT] - sbi_covt_disable_rp_ide_key(rp_id, key_desc)
