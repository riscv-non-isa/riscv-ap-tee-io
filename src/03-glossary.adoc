...
| Single-Root I/O Virtualization | SR-IOV | A PCIe specification that allows a
single physical PCIe device (such as a network interface card, GPU, or storage
controller) to present itself as multiple separate virtual devices to the
system. This enables direct hardware access from virtual machines without
requiring hypervisor intervention for I/O operations.
