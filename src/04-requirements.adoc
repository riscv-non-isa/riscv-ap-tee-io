...
optional Single-Root I/O Virtualization (SR-IOV) capabilities. When supporting
SR-IOV, the device can be split into multiple Virtual Functions (VFs). A
TVM-assigned interface can be the whole device, also known as the Physical
Device (PF), or any of its VFs. In both cases, this unit of assignment is
referred to as a TEE I/O Device Interface (TDI).
...
<<PCIE>> specification. CoVE-IO devices must support all required TDISP
requests and may support the optional ones, as defined by the TDISP request code
table.
...
supported version is 1.0 as defined in the <<PCIE>> specification.
...
RISC-V CoVE-IO compliant hosts use the Security Protocol and Data Model
(<<SPDM>>) protocol to exchange TDISP and IDE Key Management (IDE_KM) messages
with physical devices, over DOE mailboxes. CoVE-IO also relies on SPDM for
...
below PQC algorithms, as described in <<CMAPQC>>:

1. For `PqcAsymAlgo`, `ML-DSA-87`.
2. For `KEMAlg`, `ML-KEM-1024`.

CoVE-IO compatible DSMs may support following combinations during PQC transition
phase:
...
capability defined in the <<PCIE>> specification.
...
<<Smmpt>> extension, enforces that this interface is access-restricted to the
...
On platforms supporting the <<Smmpt>> and <<IO-MPT>> extensions, any PCIe device
...
* Implement the CoVE-IO host
  xref:09-coveio_abi.adoc#cove-io-host-extension-eid-0x434f5648-covh[COVH] ABI.
...
* Support the RISC-V <<IOMMU>> programming interface with an IOMMU driver.
* Support the CoVE-IO host ABI, as described in Chapter 9 of this document:
...
* Implement the CoVE-IO guest
  xref:09-coveio_abi.adoc#cove-io-guest-extension-eid-0x434f5647-covg[COVG] ABI.

===== RDSM

The Root Domain Security Manager (RDSM) is an M-mode component that retains
access to the entire physical address space and all platform resources.

To support CoVE and CoVE-IO, the RDSM must implement the Supervisor Domain
Access Protection privileged architecture extension (see <<Smmtt>>). This
extension provides the hardware enforcement mechanism necessary for isolating
security domains.

Additionally, the RDSM must implement the CoVE-IO TSM Extension
(xref:09-coveio_abi.adoc#cove-io-tsm-extension-eid-0x434f5654-covt[COVT]) ABI.
This interface enables the TSM to request privileged I/O operations, such as
programming PCIe IDE keys or configuring Root Ports, which typically require
M-mode privileges.
